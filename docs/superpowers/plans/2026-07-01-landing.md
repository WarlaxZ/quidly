...
**Goal:** A public, on-brand landing page at `/` that positions Quidly as a free, self-hostable UK-landlord tax tool (logged-in users are redirected to `/dashboard`).

**Architecture:** Make `/` (and `/icon.svg`) public in `proxy.ts`; rewrite `src/app/page.tsx` as an auth-aware server component that renders the landing (design-system tokens, so it themes light/dark automatically) or redirects authenticated users to the app.

**Tech Stack:** Next.js 16 App Router (server component), the existing Quiet Ledger CSS tokens/classes. No new deps, no binary assets.

---

## Task 1: Public landing at `/`

**Files:**
- Modify: `src/proxy.ts`
- Modify: `src/app/page.tsx`

- [ ] **Step 1: Make `/` and the favicon public in `src/proxy.ts`.**
The current line:
```ts
...
```
(Nothing else changes; all `(app)` routes stay gated.)

- [ ] **Step 2: Replace `src/app/page.tsx` with the auth-aware landing.**
```tsx
  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-6 py-6">
      ...
      <section className="reveal grid items-center gap-10 py-16 md:grid-cols-2 md:py-24" style={{ animationDelay: "60ms" }}>
        ...
        {/* Decorative ledger motif (CSS/tokens only) */}
...
- [ ] **Step 3: Verify.**
- `cd /home/ash/projects/akaunting-ng && npx tsc --noEmit 2>&1 | grep -c "error TS"` → 0 (clear `.next/dev .next/types` first if a stale generated error appears).
- `npm test` → full suite green (report totals; only `proxy.ts`/`page.tsx` changed).

- [ ] **Step 4: Commit.**
```bash
git add src/proxy.ts src/app/page.tsx
git commit -m "feat: public landing page at / (self-host positioning)"
```
...
- **Spec coverage:** public `/`+`/icon.svg` and auth-gated root (Step 1–2) ↔ spec §2; top bar / hero / features / why / self-host / footer (Step 2 markup) ↔ spec §3; verification ↔ §4.
- **Auth gate:** uses `getPrincipal()` (returns null when unauthenticated) → `redirect("/dashboard")` only for signed-in users; logged-out visitors render the landing (now reachable because `/` is in `PUBLIC_PATHS`).
- **Theme:** all colours are tokens (`bg-forest`, `text-ink`, `border-line`, `.card`, `.btn`, `.pill`, `.money`), so the page flips light/dark via the root layout's theme script — no extra work.
- **No new deps / no binary assets:** the hero "app preview" is a CSS/token ledger motif; the repo-URL is a documented `TODO` placeholder (`<a href="#">` + comment).
- **Presentation-only:** the only behavioural change is `/` access + the authed redirect target (`/transactions`→`/dashboard`); no `(app)` route or tax logic touched; the suite stays green.
